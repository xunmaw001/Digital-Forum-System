<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="讨论类型" prop="taolunleixing">
          <el-select :disabled="ro.taolunleixing" v-model="ruleForm.taolunleixing" placeholder="请选择讨论类型">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="讨论类型" prop="taolunleixing">
	      <el-input v-model="ruleForm.taolunleixing"
                placeholder="讨论类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  data() {
	...
	fabushijian : false,
      ...
      rules: {
          ...
          taolunleixing: [
                { required: true, message: '讨论类型不能为空', trigger: 'blur' },
          ],
          biaoti: [
          ],
          neirong: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fenlei = true;
          ...
          if(o=='taolunleixing'){
            this.ruleForm.taolunleixing = obj[o];
	    this.ro.taolunleixing = true;
            continue;
          }
          if(o=='biaoti'){
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../springboot1f60t/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shumataolun/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shumataolunCrossAddOrUpdateFlag = false;
						 this.parent.search();
